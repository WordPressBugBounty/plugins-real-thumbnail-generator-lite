 ...
 * @internal
 */
class Thumbnail implements IOverrideThumbnail
{
    use UtilsProvider;
    ...
    public function check($postId, $checkCap = \true, $attachedFile = null, $meta = null)
    {
        ...
        if (isset($meta['sizes']) && ($cnt = \count($meta['sizes'])) > 0) {
            ...
            foreach ($meta['sizes'] as $key => $value) {
                ...
                $thumbnailPath = \dirname($attachedFile) . '/' . $value['file'];
                ...
                if (\file_exists($thumbnailPath) && !\is_dir($thumbnailPath)) {
                    ...
                    } elseif (isset($meta['width'], $meta['height']) && ($dims = \image_resize_dimensions($meta['width'], $meta['height'], $registeredSize['width'], $registeredSize['height'], $registeredSize['crop'])) && ($shouldWidth = $dims[4]) > 0 && ($shouldHeight = $dims[5]) > 0) {
                        // If it is cropped, take the higher side and check if it is cropped already with our image
                        if ($registeredSize['crop']) {
                            $needsRegeneration = !\in_array($shouldWidth, [$value['width'], $value['height']], \true) && !\in_array($shouldHeight, [$value['width'], $value['height']], \true);
    ...
    public function get_image_sizes($fromCache = \true, $searchBy = \false, $searchWidth = null, $searchHeight = null, $searchCrop = \false)
    {
        if ($fromCache === \true && $this->imageSizes !== null && !$searchBy) {
            ...
            foreach (\get_intermediate_image_sizes() as $_size) {
                if (\in_array($_size, ['thumbnail', 'medium', 'medium_large', 'large'], \true)) {
                    ...
                    if (\function_exists('wc_get_image_size') && (Util::getInstance()->startsWith($_size, 'woocommerce_') || Util::getInstance()->startsWith($_size, 'shop_'))) {
        ...
        if ($searchBy) {
            foreach ($sizes as $key => $search) {
                if ($searchCrop && ($search['width'] == $searchWidth || $search['height'] == $searchHeight) || $search['width'] == $searchWidth && $search['height'] == $searchHeight) {
                    return [$key, $search];
                }
            }
            return \false;
    ...
    public function get_pdf_sizes($metadata, $searchBy = \false, $width = null, $height = null, $crop = \false)
    {
        ...
        if ($searchBy) {
            foreach ($sizes as $key => $search) {
                if ($crop && ($search['width'] == $width || $search['height'] == $height) || $search['width'] == $width && $search['height'] == $height) {
                    return [$key, $search];
                }
            }
            return \false;
    ...
    {
        return \trailingslashit($this->getSchemaFolder()) . $this->getSchemaFilename();
    }
    /**
